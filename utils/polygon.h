...
public:
	...
	void insertAt(int index, Point p)
	{
		(*polygon).insert((*polygon).begin() + index, p);
	}

	void set(PolygonRef newPolygon)
	{
		clear();
...
public:
	...
	//cstyle
	void unapplyMatrix(const PointMatrix& matrix)
	{
		for (unsigned int i = 0; i<polygons.size(); i++)
		{
			for (unsigned int j = 0; j<polygons[i].size(); j++)
			{
				polygons[i][j] = matrix.unapply(polygons[i][j]);
			}
		}
	}
};
