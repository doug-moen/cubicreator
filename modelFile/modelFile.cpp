...
SimpleModel* loadModelFromBuffer(const float* pBuff, int bufferSize)
{
	if (bufferSize > 0 && NULL != pBuff)
	{
		FMatrix3x3 matrix;
		SimpleModel* m = new SimpleModel();
		m->volumes.push_back(SimpleVolume());
		...
		{
			FPoint3 v0(pBuff[i], pBuff[i + 1], pBuff[i + 2]);
			FPoint3 v1(pBuff[i + 3], pBuff[i + 4], pBuff[i + 5]);
			FPoint3 v2(pBuff[i + 6], pBuff[i + 7], pBuff[i + 8]);
            Point3 vr0(0,0,0), vr1(0,0,0), vr2(0,0,0);
            vr0 = matrix.apply(v0);
            vr1 = matrix.apply(v1);
            vr2 = matrix.apply(v2);
			vol->addFace(vr0, vr1, vr2);
		}
		return m;
	}
	return NULL;
