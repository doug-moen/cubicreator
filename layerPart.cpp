...
void createLayerParts(SliceVolumeStorage& storage, Slicer* slicer, int unionAllType)
{
    for(unsigned int layerNr = 0; layerNr < slicer->layers.size(); layerNr++)
    {
        ...
        storage.layers[layerNr].z = slicer->layers[layerNr].z;
...
void dumpLayerparts(SliceDataStorage& storage, HyOutlines *pOutLines)
{
	for (unsigned int volumeIdx = 0; volumeIdx<storage.volumes.size(); volumeIdx++)
	{
		int nTotalLayer = storage.volumes[volumeIdx].layers.size();
		pOutLines->LayerSize = nTotalLayer;
		pOutLines->Layers = new HyLayer*[nTotalLayer];
		for (unsigned int layerNr = 0; layerNr < nTotalLayer; layerNr++)
		{
			SliceLayer* layer = &storage.volumes[volumeIdx].layers[layerNr];
			HyLayer* pLayer = new HyLayer();
			pOutLines->Layers[layerNr] = pLayer;

			int nPartSize = layer->parts.size();
			pLayer->PartSize = nPartSize;
			pLayer->Parts = new HyPart*[nPartSize];

			for (unsigned int i = 0; i<nPartSize; i++)
			{
				SliceLayerPart* part = &layer->parts[i];
				HyPart *pPart = new HyPart();
				pLayer->Parts[i] = pPart;

				int nOutlineSize = part->outline.size();
				pPart->Polygons = new HyPolygon*[nOutlineSize];
				pPart->PolygonSize = nOutlineSize;

				for (unsigned int j = 0; j<nOutlineSize; j++)
				{
					HyPolygon *pPolygon = new HyPolygon();
					pPart->Polygons[j] = pPolygon;
					int nDataSize = part->outline[j].size() * 2;
					pPolygon->Size = nDataSize;
					pPolygon->Data = new float[nDataSize];

					for (unsigned int k = 0; k < part->outline[j].size(); k++)
					{
						pPolygon->Data[k * 2] = part->outline[j][k].X;
						pPolygon->Data[k * 2 + 1] = part->outline[j][k].Y;
					}
						//fprintf(out, "%f,%f ", float(part->outline[j][k].X - modelMin.x) / modelSize.x * 500, float(part->outline[j][k].Y - modelMin.y) / modelSize.y * 500);
				}
			}
		}
		// TODO: support first volume, yet.
		break;
	}
}