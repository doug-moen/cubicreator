...
typedef struct HyPolygon
{
	float *Data;
	int Size; // size of x, y position sets.
};
...
typedef struct HyLayer
{
	int PartSize;
	HyPart **Parts;
};

typedef struct HyOutlines
{
	int LayerSize;
	HyLayer **Layers;
};
...
typedef struct SModel
{
	int size;
	float *pBuff;
	float posX; // config.objectPosition.X
	float posY; // config.objectPosition.Y
	float posZ; // config.objectSink
	float initialLayerThickness; // initialLayerThickness
	float layerThickness; //layerThickness
	int fixHorrible; // fixHorrible
	int reserved;
	HyOutlines *pOutLines;
};
...
public:
	...
	static SliceDataStorage * Slice(SliceDataStorage *pStorage, SModel *pModel, float sliceZ, bool keepNoneClosed, bool extensiveStitching);
	static void Slice(SliceDataStorage *pStorage, HyOutlines *pOutLines);
