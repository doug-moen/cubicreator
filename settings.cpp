...
ConfigSettings::ConfigSettings()
{
	...
	SETTING(skirtSpeed, 20);
	SETTING(startSpeed, 20);
    SETTING(fanFullOnLayerNr, 2);

	SETTING(supportInfillType, SUPPORT_TYPE_LINES_END_CONNECT);
	...
	SETTING(retractRestoreCorrection, 0);//cstyle
	...
	SETTING(raftType, RAFT_TYPE_CUBICON);
	...
	SETTING(raftPlaneWidth1, 200);
	SETTING(raftPlaneWidth2, 300);
	SETTING(raftPlaneThickness1, 100);
	SETTING(raftPlaneThickness2, 200);
	SETTING(raftBaseSpeed, 40);
	SETTING(raftInterfaceSpeed, 10);
	...
	SETTING(gcodeFlavor, GCODE_FLAVOR_CUBICREATOR);// GCODE_FLAVOR_REPRAP);
	...
	//additional option
	//cstyle raft dash setting
	SETTING(raftDashStride, 1900);
	SETTING(raftDashSize, 1500);
	SETTING(wallInOutOrder, 0);
	SETTING(insetEndRetraction, 0);
	SETTING(strongSupportLayerNr, 1);

	SETTING(wipetype, 0);
	SETTING(wipePosition.X, 0);
	SETTING(wipePosition.Y, 0);
	SETTING(wipePosition1.X, 0);
	SETTING(wipePosition1.Y, 0);
	SETTING(wipeExtrusionAmount, 5000);
	SETTING(finalizeRetraction, 1);
	SETTING(fanspeedThreshold, 1);
	SETTING(airGap, 350);
	SETTING(layerSupport, 1);
	SETTING(layerSupportTopMarginCount, 3);
	SETTING(homelineThickness, 400);	

	SETTING(homeline_marginX, -1000);//-1mm
	SETTING(homeline_marginY, -1000);//-1mm
	SETTING(homeline_startX, 16750);
	SETTING(homeline_startY, 4000);
	SETTING(bed_width, 240000);
	SETTING(bed_height, 190000);

	SETTING(innerWallEndControlType, 0);
	SETTING(innerwallEndReduceRate, 100);
	SETTING(innerWallEndGap, 0);
	SETTING(outerWallEndGap, 0);
	SETTING(pullEndOfWallTravelDistance, 0);
	SETTING(pullEndOfWallAngle, -90);
	SETTING(travelSpeedMatching, 0);
	SETTING(outerWallStartOffset, 0);
	SETTING(fixedWallBeginAngle, -999);

	SETTING(finalizeExtLift, 1000);
	SETTING(initialLayerFillType, 0);	
...
bool ConfigSettings::setSetting(const char* key, const char* value)
{
	...
	//cstyle
	std::vector<std::string> data = AnalyzeArgument(std::string(key));
	if (data.size()>1 && strcasecmp(data[0].c_str(), "layerControl") == 0)
	{
		layerControls.push_back(LayerControl(atoi(data[1].c_str()), value));	
		return true;
	}
    return false;
}
...
std::vector<std::string> ConfigSettings::AnalyzeArgument(std::string str)
{
	// Split into key = val
	std::vector<std::string> codes;
	std::string opcode(""), val("");
	size_t pos = str.find_first_of(':');
	if (pos != std::string::npos && str.length() > (pos + 1))
	{
		opcode = str.substr(0, pos);
		val = str.substr(pos + 1);
		TRIM_STRING(opcode);
		TRIM_STRING(val);
		codes.push_back(opcode);
		codes.push_back(val);
	}
	else
	{
