...
{
public:
	int vertex_index[3];// = { -1, -1, -1 }; //!< counter-clockwise ordering
    int connected_face_index[3]; //!< same ordering as vertex_index (connected_face 0 is connected via vertex 0 and 1, etc.)

	MeshFace() //:vertex_index{-1, -1, -1}
	{
		//Array<5, int, -1, -1, -1> a(vertex_index);
		vertex_index[0] = -1;
		vertex_index[1] = -1;
		vertex_index[2] = -1;
...
public:
	...
	MeshGroup *mpMeshGroup;
	Mesh(MeshGroup *pMeshGroup);// SettingsBaseVirtual* parent); //!< initializes the settings
